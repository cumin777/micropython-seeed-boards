...
- 2026-06-04: Implemented the final product-side fix for missing `boards` uploads by freezing the `boards` helper package into the Zephyr firmware. Added `lib/micropython/ports/zephyr/modules/boards/{__init__,xiao,xiao_nrf54lm20a}.py`, enabled `CONFIG_MICROPY_FROZEN_MODULES`, resolved the Windows `mpy-cross` dependency using an installed `mpy-cross` executable, rebuilt the firmware successfully, and reflashed the currently connected board with the updated image.
- 2026-06-04: Restored and fixed `example/test_receiver.py` after noticing the repository copy had been deleted locally. The script now uses a `sys.stdin.readline()` prompt helper instead of `input()`, matching the capabilities of the current Zephyr MicroPython port, and the active validation copy under `D:\workspace\memory\xiao\20a\micropython\` was updated in sync.
- 2026-06-04: Patched the `xiao_nrf54lm20a` board definition to drive the IMU CS line high on `P3.12` using a `gpio-hog`, ensuring the LSM6DS3TR-C stays in I2C mode. Rebuilt the firmware successfully, refreshed the flash package artifacts, and reflashed the connected board with the updated image.
- 2026-06-04: Fixed the PWM test item in `test_receiver.py` to match the current Zephyr MicroPython PWM API. The first `init()` call now sets both `freq` and `duty_ns`, avoiding the runtime error `Frequency and duty values must be set`.
